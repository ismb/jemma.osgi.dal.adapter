...
public class EnergyMeterDALAdapter extends BaseDALAdapter implements Meter {
	...
	private void updateDivisor() throws Exception {
		if (divisor == null) {
			divisor = (Integer) this.appliancesProxy.invokeClusterMethod(appliancePid, endPointId, SIMPLEMETERINGCLUSTER, "getDivisor",
					createParams(SIMPLEMETERINGCLUSTER, "getDivisor", new String[0]));
		}

		// Work-around: Indesit White goods does not report divisor and
		// multiplier
		if (divisor == 0) {
			divisor = 10000;
		}
	...
	private void updateMultiplier() throws Exception {
		if (multiplier == null) {
			multiplier = (Integer) this.appliancesProxy.invokeClusterMethod(appliancePid, endPointId, SIMPLEMETERINGCLUSTER, "getMultiplier",
	...
	public LevelData getCurrent() throws UnsupportedOperationException, IllegalStateException, DeviceException {
		...
		try {
			instantaneousDemand = (Integer) this.appliancesProxy.invokeClusterMethod(appliancePid, endPointId, SIMPLEMETERINGCLUSTER, "getIstantaneousDemand",
	...
	public LevelData getTotal() throws UnsupportedOperationException, IllegalStateException, DeviceException {
		BigDecimal result = null;
		long total;
		// According to meter type obtained from SERVICE_FLOW property, the
		// correct cluster method is called
		try {
	...
	private BigDecimal scaleValues(BigDecimal value) throws Exception {
		BigDecimal factor = new BigDecimal(getMultiplier()).divide(new BigDecimal(getDivisor())) // now
																									// I
																									// have
																									// the
																									// factor
