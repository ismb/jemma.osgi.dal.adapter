 ...
 * <ul>
 * <li>IApplicationService to get notifications from JEMMA of configured or
 * deleted appliances</li>
 * <li>IAttributeValueListener to receive Attributes reportings from a device
 * and make them become OSGi DAL events</li>
 * <li>IManagedAppliance to be a virtual appliance exposing Client Clusters
 * which are needed to receive commands from appliances</li>
 * </ul>
 * 
...
public class ZigBeeDalAdapter extends Appliance implements IApplicationService, IAttributeValuesListener, ApplianceControlClient {
	...
	// Parameters to configure this class as appliance with exposed endpoints.
	// Usefult to get notification from Server Clusters
	...
	public ZigBeeDalAdapter() throws ApplianceException {
		...
		// TODO: evaluate dynamic discovery of factories using Declarative
		// services
		// add factories
	...
	/**
	 * Method called by JEMMA when a new appliance is configured in the
	 * framework
	 * 
	 * @param endPoint
	 * @param appliance
	...
	public void notifyApplianceAdded(IApplicationEndPoint endPoint, IAppliance appliance) {
		...
		for (IEndPoint ep : appliance.getEndPoints()) {
			for (IServiceCluster cluster : ep.getServiceClusters()) {
				if (factories.containsKey(cluster.getName())) {
					...
					addFunctionRegistration(appliance.getPid(), factories.get(cluster.getName()).createFunctionService(appliance, ep.getId(), appliancesProxy));
		...
		devices.put(appliance.getPid(), FrameworkUtil.getBundle(this.getClass()).getBundleContext().registerService(Device.class.getName(), new JemmaDevice(), d));
	...
	/**
	 * Internal method storing service registrations for future use (e.g.
	 * service de-registration when appliance is removed)
	 * 
	 * @param appliancePid
	 * @param registration
	 */
	...
	/**
	 * Updates device services properties (e.g. Device status) according to the
	 * content of the IAppliance class
	 * 
	 * @param appliance
	 */
	...
	/**
	 * Method invoked by JEMMA when an attribute is received from an appliance
	 * through ZigBee AttributeNotification services
	 */
	public void notifyAttributeValue(String appliancePid, Integer endPointId, String clusterName, String attributeName, IAttributeValue attributeValue) {

		LOG.info("Received an attribute {} from {}", attributeName, clusterName);
		...
		// update device service properties status if they are changed: JEMMA is
		// lazy notifying availability changes
		...
		// Filter
		// filter=FrameworkUtil.getBundle(this.getClass()).getBundleContext().createFilter(filterString);
		BundleContext ctx;
		...
		try {
			...
			// get the DAL property name related to this attribute from the
			// factory related to the cluster which generated the attribute
			...
			// now convert the attribute to a FunctionData using the resolved
			// Function service
	...
	// TODO evaluate how to re-implement this putting logic for client clusters
	// in factories or functions
	...
	public void execSignalStateNotification(short ApplianceStatus, short RemoteEnableFlags, int ApplianceStatus2, IEndPointRequestContext context) throws ApplianceException,
		...
		try {
			...
			// so a Function implementing interactions with
			...
			functionRefs = (ServiceReference[]) componentContext.getBundleContext().getServiceReferences(Function.class.getName(), filterString);
			...
			// invoke method that create FunctionData according to received
			// property and post the event
